	...
	try:
			...
			if not uid in queue["occupied"]:
				...
				for tiktok in trending:
					...
					inline = InlineKeyboardMarkup(inline_keyboard=[
						[InlineKeyboardButton(text="FOLLOW INSTAGRAM", url='https://www.instagram.com/always_bacot'), InlineKeyboardButton(text="GRUP CHAT", url="https://t.me/SquadAB")]])
		...
		if text == "/test":
			...
			if not uid in queue["occupied"]:
				...
				bot.sendMessage(uid, '_Mencari teman halu kamu.. tunggu sebentar_',parse_mode='MarkDown', reply_markup=keyboard)
			...
			bot.sendMessage(uid, "Welcome BOT mblo\nYuk Join My Grup @SquadAB :)", reply_markup=keyboard)
		...
		if len(queue["free"]) > 1 and not uid in queue["occupied"]:
			...
			if partner != uid:
				...
				bot.sendMessage(uid, '_🎈Pasangan kamu telah ditemukan, cieee wkwkwk_',parse_mode='MarkDown', reply_markup=keyboard)
				bot.sendMessage(partner, '_🎈Pasangan kamu telah ditemukann, cieee wkwkwk_',parse_mode='MarkDown', reply_markup=keyboard)
